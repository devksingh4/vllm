...
def test_chunked_local_attention_remove_skipped_blocks():
    ...
    manager.remove_skipped_blocks("test", 0, "test-partition")
    assert_block_id(block_table, original_block_ids)

    # For 4th token (0-indexed), token 0-3 is out of the local attention window.
    manager.remove_skipped_blocks("test", 4, "test-partition")
    ...
    manager.remove_skipped_blocks("test", 6, "test-partition")
    assert_block_id(block_table, [null_block_id] * 2 + original_block_ids[2:])
    # For 12th token (0-indexed),
    # token 0-11 are out, 6 block can be removed.
    manager.remove_skipped_blocks("test", 12, "test-partition")
...
def test_sliding_window_remove_skipped_blocks():
    ...
    manager.remove_skipped_blocks("test", 5, "test-partition")
    ...
    manager.remove_skipped_blocks("test", 7, "test-partition")
    assert_block_id(block_table, [null_block_id] * 2 + original_block_ids[2:])
    ...
    manager.remove_skipped_blocks("test", 11, "test-partition")
